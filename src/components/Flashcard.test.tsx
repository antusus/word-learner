...
describe('Flashcard', () => {
  ...
  it('applies flipped class when isFlipped is true', () => {
    const { container } = render(<Flashcard word={mockWord} isFlipped={true} onFlip={() => {}} />)
    expect(container.querySelector('.flashcard.flipped')).toBeInTheDocument()
  })

  it('does not have flipped class when isFlipped is false', () => {
    const { container } = render(<Flashcard word={mockWord} isFlipped={false} onFlip={() => {}} />)
    expect(container.querySelector('.flashcard.flipped')).not.toBeInTheDocument()
  })
})
