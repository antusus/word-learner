...
describe('useProgress', () => {
  ...
  it('does not duplicate completed units', () => {
    ...
    expect(
      result.current.completedUnits.filter((id) => id === 'Unit1'),
    ).toHaveLength(1);
  });
  ...
  it('persists progress to localStorage', () => {
    ...
    expect(JSON.parse(stored as string)).toEqual({ completedUnits: ['Unit1'] });
  });

  it('loads progress from localStorage', () => {
    localStorage.setItem(
      STORAGE_KEY,
      JSON.stringify({ completedUnits: ['Unit1', 'Unit2'] }),
    );
  ...
  it('handles invalid data structure in localStorage', () => {
    localStorage.setItem(
      STORAGE_KEY,
      JSON.stringify({ completedUnits: 'not an array' }),
    );
